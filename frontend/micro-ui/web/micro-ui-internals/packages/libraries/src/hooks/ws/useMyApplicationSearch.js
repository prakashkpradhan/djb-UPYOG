...
const useMyApplicationSearch = ({ tenantId, filters = {}, BusinessService = "WS", t }, config = {}, getWorkflow = false) => {
  ...
  const { isLoading: isWSLoading, error: isWSError, data: wsData, isSuccess: wsSuccess } = useQuery(
    ["WS_SEARCH_CONNECTION", tenant, BusinessService, data, config],
    async () =>
      await WSService.search({
        tenantId: tenant,
        filters:
  ...
  const { isLoading: isWorkflowLoading, data: workflowData } = useQuery(
    ["WS_SEARCH_WROKFLOW", tenant, wsData],
    async () => await Digit.WorkflowService.getByBusinessId(tenant, businessIds.join(",")),
    {
