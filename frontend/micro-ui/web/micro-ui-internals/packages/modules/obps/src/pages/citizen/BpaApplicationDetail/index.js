...
const BpaApplicationDetail = () => {
  ...
  if (data?.applicationDetails?.length > 0) {
    data.applicationDetails =
      data?.applicationDetails?.length > 0 && data?.applicationDetails?.filter((bpaData) => Object.keys(bpaData).length !== 0);
  }
      ...
      {data?.applicationDetails
        ...
        .map((detail, index, arr) => {
          return (
            <div>
              {!detail?.isNotAllowed ? (
                <Card key={index} style={!detail?.additionalDetails?.fiReport && detail?.title === "" ? { marginTop: "-30px" } : {}}>
                  ...
                  >
                    <StatusTable>
                        ...
                        <div style={{ marginTop: "-8px" }}>
                          {
                            <DocumentsPreview
                              ...
                              isHrLine={true}
                              titleStyles={{ fontSize: "20px", lineHeight: "24px", fontWeight: 700, marginBottom: "10px" }}
                            />
                          }
                        </div>
                      )}
                                ...
                                <Row
                                  ...
                                  textStyle={
                                    detail?.values?.[1]?.value == "APPROVED" || detail?.values?.[1]?.value == "AUTO_APPROVED"
                                      ? { marginLeft: "10px", color: "#2E9E8F" }
                                      : { marginLeft: "10px", color: "#D83A2F" }
                                  }
                      ...
                      {!detail?.isTitleVisible && detail?.additionalDetails?.permit?.length > 0
                        ? detail?.additionalDetails?.permit?.map((value) => <CardText>{value?.title}</CardText>)
                        : null}
                    ...
                    {!workflowDetails?.isLoading && workflowDetails?.data?.nextActions?.length > 1 && (
                      ...
                      <ActionBar /*style={{ position: "relative", boxShadow: "none", minWidth: "240px", maxWidth: "310px", padding: "0px" }}*/>
                        <div style={{ width: "100%" }}>
                          ...
                          <SubmitBar
                            /*style={{ width: "100%" }}*/ disabled={checkForSubmitDisable(isFromSendBack, isTocAccepted)}
