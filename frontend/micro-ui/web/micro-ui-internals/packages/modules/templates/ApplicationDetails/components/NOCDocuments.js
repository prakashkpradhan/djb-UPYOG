...
import { CardLabel, MultiUploadWrapper, StatusTable, Row, LabelFieldPair } from "@djb25/digit-ui-react-components";
import DocumentsPreview from "./DocumentsPreview";

function SelectDocument({ t, document: doc, setNocDocuments, setError, nocDocuments }) {
  ...
  return (
    <div
      style={
        {
          /*  border: "1px solid #D6D5D4", padding: "16px 0px 16px 8px", background: "#FAFAFA", borderRadius: "5px", marginBottom: "24px", display: "flex" */
        }
      }
    >
...
const NOCDocuments = ({ t, noc, docs, isNoc, applicationData, NOCdata, bpaActionsDetails }) => {
  ...
  useEffect(() => {
    ...
    if (isNoc) {
      ...
      filteredData = bpaDocs?.BPA?.DocTypeMapping?.filter(
        (data) =>
          data.WFState == applicationData?.status &&
          data.RiskType == applicationData?.riskType &&
          data.ServiceType == applicationData?.additionalDetails?.serviceType &&
          data.applicationType == applicationData?.additionalDetails?.applicationType
      );
    }
  ...
  return (
    <div
      style={{
        ...
        borderRadius: "5px",
        marginBottom: "24px",
        maxWidth: "950px" /*  display: "flex" */,
      }}
    >
      <StatusTable>
        <Row
          label={isEmployee ? `${t(`BPA_${noc?.nocType}_HEADER`)}` : t(`BPA_${noc?.nocType}_HEADER`)}
          labelStyle={{ fontSize: "20px", width: "150%" }}
        />
        {NOCdata &&
              ...
              if (noc?.field == "STATUS") {
                return (
                  <Row
                    ...
                    label={isEmployee ? `${t(noc?.title)}` : t(noc?.title)}
                    text={noc?.value ? t(noc?.value) : t("CS_NA")}
                    textStyle={noc?.value == "APPROVED" || noc?.value == "AUTO_APPROVED" ? { color: "#2E9E8F" } : { color: "#D83A2F" }}
                  />
                );
              } else {
                return (
