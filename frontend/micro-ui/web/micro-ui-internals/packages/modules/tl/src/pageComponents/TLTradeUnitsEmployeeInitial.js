...
import { CardLabel, LabelFieldPair, Dropdown, TextInput, LinkButton, CardLabelError, MobileNumber } from "@djb25/digit-ui-react-components";
...
const TLTradeUnitsEmployeeInitial = ({ config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  ...
  const { data: billingSlabTradeTypeData, isLoading } = Digit.Hooks.tl.useTradeLicenseBillingslab(
    { tenantId, filters: {} },
    {
      select: (data) => {
  ...
  return (
    <React.Fragment>
      ...
      <LinkButton label={t("TL_ADD_TRADE_UNIT")} onClick={addNewUnits} style={{ color: "#3A8DCC", width: "fit-content" }} />
    </React.Fragment>
  );
};
...
const TradeUnitForm = (_props) => {
  ...
  useEffect(() => {
    if (
      ...
      let filteredTradeDetails = billingSlabTradeTypeData.filter(
        (data) => data?.structureType === formData?.tradedetils?.["0"]?.structureSubType?.code.toString()
      );
        ...
        <div style={{ border: "1px solid #D6D5D4", padding: "16px", marginTop: "8px", background: "#FAFAFA" }}>
          ...
          <LabelFieldPair>
            <CardLabel className="card-label-smaller">
              {unit?.tradeSubType?.uom ? `${t("TL_NEW_TRADE_DETAILS_UOM_UOM_PLACEHOLDER")} * ` : `${t("TL_NEW_TRADE_DETAILS_UOM_UOM_PLACEHOLDER")}`}
            </CardLabel>
            <div className="field">
              <Controller
                control={control}
              ...
              <Controller
                ...
                rules={
                  unit?.tradeSubType?.uom && {
                    ...
                    validate: {
                      pattern: (val) =>
                        /*/^(0)*[1-9][0-9]{0,5}$/.test(val)*/ val > 0 && val < 99999
                          ? checkRangeForUomValue(val, unit?.tradeSubType?.fromUom, unit?.tradeSubType?.toUom)
                            ? true
                            : `${t("ERR_WRONG_UOM_VALUE")} ${unit?.tradeSubType?.fromUom} - ${unit?.tradeSubType?.toUom}`
