...
const ApplicationOverview = () => {
    ...
    if (nocDataDetails?.length && nocDocumentsList?.length && nocDataDetails?.[0] != undefined) {
      ...
      return (
        <Fragment>
          ...
          >
            <CardSectionHeader style={{ marginBottom: "16px", fontSize: "20px" }}>{`${t(
              `NOC_MAIN_${stringReplaceAll(nocDocumentsList?.[0]?.code, ".", "_")}_LABEL`
            )}`}</CardSectionHeader>
            <StatusTable style={{ position: "relative", marginTop: "19px" }}>
              <Row
                className="border-none"
                label={`${t(`NOC_${nocDataDetails?.[0]?.nocType}_APPLICATION_LABEL`)}`}
                text={t(nocDataDetails?.[0]?.applicationNo) || "NA"}
              />
              <Row
                className="border-none"
                ...
                text={t(status) || "NA"}
                textStyle={
                  nocDataDetails?.[0]?.applicationStatus == "APPROVED" || nocDataDetails?.[0]?.applicationStatus == "AUTO_APPROVED"
                    ? { color: "#2E9E8F" }
                    : { color: "#D83A2F" }
                }
              />
              <Row
                ...
                text={
                  nocDataDetails?.[0]?.additionalDetails?.SubmittedOn
                    ? convertEpochToDate(Number(nocDataDetails?.[0]?.additionalDetails?.SubmittedOn))
                    : "NA"
                }
              />
              ...
              <Row
                ...
                text={
                  status === "APPROVED" || status === "REJECTED" || status === "AUTO_APPROVED" || status === "AUTO_REJECTED"
                    ? convertEpochToDate(Number(nocDataDetails?.[0]?.auditDetails?.lastModifiedTime))
                    : "NA"
                }
              />
              ...
              <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "flex-start" }}>
                {nocDataDetails?.[0]?.documents?.map((value, index) => (
                  ...
                  >
                    ...
                    <p style={{ marginTop: "8px", fontWeight: "bold", textAlign: "center", width: "100px", color: "#505A5F" }}>
                      {
                        t(
                          `NOC_MAIN_${stringReplaceAll(nocDocumentsList?.[0]?.code, ".", "_")}_LABEL`
                        ) /* t(value?.title ? value?.title : decodeURIComponent( pdfFiles[value.fileStoreId]?.split(",")?.[0]?.split("?")?.[0]?.split("/")?.pop()?.slice(13))) */
                      }
                    </p>
                  </a>
                ))}
  ...
  const getBuldingComponent = (details = []) =>
    details.map((detail) => ({
      title: detail.title,
      belowComponent: () => (
        <Fragment>
          <div style={{ maxWidth: "950px" }}>
            <StatusTable style={{ position: "relative", marginTop: "19px" }}>
              {detail.values.map((value) => (
                <Row className="border-none" label={`${t(value?.title)}`} text={value?.value || "NA"} />
              ))}
            </StatusTable>
          </div>
        </Fragment>
  ...
  useEffect(() => {
    if (applicationDetails) {
      ...
      setAppDetails({
        ...applicationDetails,
        applicationDetails: [
          getBuldingComponent(details)?.[0],
          {
            title: "NOC_DETAILS_SUMMARY_LABEL",
            belowComponent: () => <DocumentDetails t={t} data={applicationDetails} nocDataDetails={nocDatils} nocDocumentsList={nocTaxDocuments} />,
          },
        ],
      });
...
function SelectDocument({ t, document: doc, setNocDocuments, error, setError, nocDocuments }) {
