...
const WSRoadCuttingDetails = ({ config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  ...
  const removeroadCutt = (roadCutt) => {
    roadCutt?.id !== null
      ? setRoadCuttingDetails((prev) =>
          prev.map((e) => (e.key == roadCutt.key && e.id !== null ? { ...roadCutt, id: e.id, status: "INACTIVE" } : { ...e }))
        )
      ...
      <div>
        ...
        <div style={{ marginTop: "8px" }}>
            ...
            <LinkButton
              label={
                <DeleteIcon
                  ...
                  fill={!(allRoadCuttingDetails?.length == 1) ? "#494848" : "#FAFAFA"}
                />
              }
          ...
          <div
            ...
            style={allRoadCuttingDetails?.filter((o) => o.status !== "INACTIVE").length == 1 ? {} : { marginTop: "40px" }}
          >
            <div>
              <LabelFieldPair>
              ...
              {localFormState.touched?.roadType ? <CardLabelError style={errorStyle}> {errors?.roadType?.message}</CardLabelError> : null}
            </div>
            <div>
              <LabelFieldPair>
              ...
              {localFormState.touched?.area ? <CardLabelError style={errorStyle}> {errors?.area?.message}</CardLabelError> : null}
            </div>
          </div>
        </div>
      </div>
